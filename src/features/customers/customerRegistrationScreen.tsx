import { AlertDialog, AlertDialogAction, AlertDialogContent, AlertDialogDescription, AlertDialogFooter, AlertDialogHeader } from "@/components/ui/alertDialog";
...
import { z } from "zod";
import { postRegisterCustomer } from "./customerService";
...
const customerRegistrationSchema = z.object({
  ...
  birthdate: z
    .date()
    .refine((date) => isValid(date) && differenceInYears(new Date(), date) >= 18, {
...
export default function CustomerRegistrationScreen() {
  ...
  const { control, register, handleSubmit, formState: { errors } } = useForm<CustomerRegistrationFormData>({
  ...
  const onSubmit = async (data: CustomerRegistrationFormData) => {
    try {
      await postRegisterCustomer(data);
      setIsSuccess(true); 
    } catch (error) {
  ...
  return (
    <div className="max-h-screen flex flex-col bg-gray-100">
      ...
      <div className="w-full px-20">
        <form onSubmit={handleSubmit(onSubmit)} className="w-full max-w-5xl bg-white p-10 rounded-lg shadow-md mx-auto">
...
/>
          ...
          <div className="mt-4 max-w-xs flex gap-2">
            <Input {...register("ddd")} placeholder="DDD" maxLength={3} className="w-20" />
            <Input {...register("phone")} placeholder="Telefone" type="tel" className="w-60" maxLength={9} />
          </div>
          {errors.ddd && <p className="text-red-500 text-sm">{errors.ddd.message}</p>}
          {errors.phone && <p className="text-red-500 text-sm">{errors.phone.message}</p>}

          <div className="flex justify-center mt-6 gap-[50px] ">
            <Button type="button" variant="outline" onClick={() => navigate("/listaClientes")} className="rounded-xl bg-neutral-300 text-white">
              Cancelar
            </Button>
